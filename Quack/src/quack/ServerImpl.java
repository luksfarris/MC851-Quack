...
import 
    quack.User, quack.UserTable,
    quack.Session, quack.SessionTable, 
    quack.Message, quack.MessageTable,
    quack.Contact, quack.ContactTable,
    quack.Database,
    quack.HTML;
...
public class ServerImpl implements Server {
    ...
    ContactTable contactTable = null;  // Usuários seguidos, bloqueados, etc. de cada usuário.
    ...
    public String processRegistrationReq(String userName, String email, String fullName, String password) {
        ...
        if (user != null) { return HTML.errorPage("there is already a user account with that email"); }
        
        // Cria o usuário e acrescenta à tabela:
        user = new UserImpl();
        user.initialize(userName, email, fullName, password);
        if (user == null) { return HTML.errorPage("user creation failed for unknown reason"); }
    ...
    public String processLoginReq(String userName, String password) { 
        ...
        User user = this.userTable.fromUserName(userName);
        if (user == null) { return HTML.errorPage("no such user"); }
        // Verifica se a senha bate:
        if (! user.checkPassword(password)) { return HTML.errorPage("wrong password"); }
    ...
    public String processLogoutReq(String cookie) {
        ...
        if (session == null) { return HTML.errorPage("no session with this cookie."); }
        // Fecha a sessão existente: 
        this.sessionTable.delete(session);
        session.close();
